"""
Interactive 3D decompressed-token CFG — Emerald Tablet.

Outputs a self-contained HTML file openable in any browser.
Full mouse rotation, zoom, pan, hover tooltips.

Layout:
  X, Y  — 3D spring layout (networkx, dim=3); nodes cluster by connection
  Z     — same spring axis; all three axes are physics-derived
...
Arrow cones are placed at edge midpoints for directed edges with
non-trivial semantic types (∅, W, ←, src, δ).
...
# ── Plotly-friendly edge colours (same palette as 2D) ─────────────────────
...
def build_3d_traces(C: nx.DiGraph, pos3d: dict) -> list[go.BaseTraceType]:
    ...
    # ── Edge traces — one trace per etype ────────────────────────────────
    ...
    # ── Arrow cones at midpoints for semantic edges ───────────────────────
    cone_etypes = {"weighted", "backpop", "empty", "frobenius", "source"}
    for etype in cone_etypes:
        ...
        # Parse rgba string to get colour for cone
    ...
    for n in node_list:
        ...
        out_e = [(d.get("etype",""), d.get("label","")) for _, _, d in C.out_edges(n, data=True)]
        in_e  = [(d.get("etype",""), d.get("label","")) for _, _, d in C.in_edges(n, data=True)]
        out_str = "  ".join(f"{lbl}→%r{v}" for (et,lbl),(_, v, _) in
                            zip(out_e, C.out_edges(n, data=True))) if out_e else "—"
        in_str  = "  ".join(f"%r{u}→{lbl}" for (et,lbl),(u, _, _) in
                            list(zip(in_e, C.in_edges(n, data=True)))[:4]) if in_e else "—"
        hover_texts.append(
            f"<b>%r{n}  {mnem}</b><br>"
            f"family: {fam}<br>"
...
def main() -> None:
    ...
    fig.update_layout(
        title=dict(
            text=(
                f"Emerald Tablet — Decompressed Token CFG (3D Interactive)<br>"
