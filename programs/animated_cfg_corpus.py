"""
Animated full-corpus IMASM CFG — Emerald Tablet.

Uses the complete compiled instruction stream (all versicles concatenated).
Register ID aliasing across versicles creates genuine cycles: cross-versicle
back-edges form the looping nested structure.

Animation:
  Phase 1 — build: nodes appear in instruction order; versicle-boundary
             crossings flash white; hub nodes grow large.
  Phase 2 — flow wave: Gaussian pulse travels through execution order;
             cross-versicle back-edges light up amber when the wave fires
...
_VERSICLE_FLASH  = np.array(mcolors.to_rgba("#cc44ff"))
...
def render_frame(
    ...
    crossversicle_edge_set: set[tuple[int, int]],
    ...
    if revealed is not None:
        for u, v, d in edges:
            if u not in revealed or v not in revealed:
                continue
            is_cv = (u, v) in crossversicle_edge_set or (v, u) in crossversicle_edge_set
        ...
        if versicle_flash:
            colors = np.ones_like(colors) * _VERSICLE_FLASH
            sizes  = sizes * 1.8
        ...
        active_cv_nodes: set[int] = set()
        for idx, n in enumerate(all_nodes):
            if weights[idx] > 0.4:
                active_cv_nodes.add(n)

        for u, v, d in edges:
            is_cv = (u, v) in crossversicle_edge_set or (v, u) in crossversicle_edge_set
            is_fr = mnem_map.get(u, "") in _FROBENIUS or mnem_map.get(v, "") in _FROBENIUS
            near  = u in active_cv_nodes or v in active_cv_nodes
        ...
        for i, n in enumerate(all_nodes):
            w = weights[i]
            is_hub_cv = any(
                (n, nb) in crossversicle_edge_set or (nb, n) in crossversicle_edge_set
                for nb in list(active_cv_nodes)[:8]
            )
            target = _PULSE_AMBER if is_hub_cv and w > 0.2 else _PULSE_WHITE
...
def main(
    ...
    sections = result["versicles"]
    section_count = result["versicle_count"]
    print(f"  {section_count} versicles, {result['total_registers']} total registers")
    ...
    crossversicle_edge_set: set[tuple[int, int]] = set()
    for u, v, d in C.edges(data=True):
        if v < u:
            crossversicle_edge_set.add((u, v))
    print(f"  Cross-versicle back-edges: {len(crossversicle_edge_set)}")
    ...
    versicle_node_positions: set[int] = set()
    for bi in boundaries:
        if bi < len(parsed):
            r = parsed[bi][0]
            if r in seen and r in node_order:
                versicle_node_positions.add(node_order.index(r))
    ...
    for f in range(total_frames):
        ...
        if f < build_frames:
            ...
            flash    = any(abs(k - fp) <= 1 for fp in versicle_node_positions)
            vers_idx = sum(1 for bi in boundaries if (bi // max(1, len(parsed) // N)) < k)
            title = (
                f"Emerald Tablet — full corpus | build {k}/{N} nodes | "
                f"versicle ~{vers_idx}/{section_count} | "
                f"{len(crossversicle_edge_set)} cross-versicle loops"
            )
            render_frame(
                ax, node_order, pos, edges, mnem_map,
                base_colors, base_sizes, crossversicle_edge_set,
                revealed=revealed, versicle_flash=flash,
            ...
            title = (
                f"Emerald Tablet — full corpus | flow wave | "
                f"%r{node_at} {mnem_at} | "
                f"{len(crossversicle_edge_set)} cross-versicle loops"
            )
            render_frame(
                ax, node_order, pos, edges, mnem_map,
                base_colors, base_sizes, crossversicle_edge_set,
                revealed=None, versicle_flash=False,
