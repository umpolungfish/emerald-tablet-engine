#!/usr/bin/env python3
"""
Bootstrap Cycle Explorer — Frobenius loops in the Emerald Tablet corpus.
...
Performs:
  ...
  - Bootstrap sequence closure detection per versicle
  - Section-level cycle density comparison (proem/cosmogony/praxis/closure)
  - Spectral gap of the transition matrix

Usage:
    python programs/bootstrap_explorer.py data/emerald_tablet_etff.txt
    python programs/bootstrap_explorer.py data/emerald_tablet_etff.txt --max-mismatches 2
"""
...
from emerald_tablet_engine import compile_corpus, PRIMITIVES, BOOTSTRAP_SEQUENCE, SECTIONS
...
def extract_code_stream(result: dict) -> list[str]:
    stream = []
    for v_name in sorted(result['versicles'].keys(),
                          key=lambda x: int(re.sub(r'\D', '', x) or 0)):
        for instr in result['versicles'][v_name]['instructions']:
...
def main():
    ap = argparse.ArgumentParser(description='Bootstrap Cycle Explorer — Emerald Tablet')
    ap.add_argument('transcription', help='Path to Emerald Tablet ETFF file')
    ...
    print(f"\nEmerald Tablet Bootstrap Cycle Explorer")
    print(f"{'─' * 60}")
    ...
    for start, end, miss in windows[:10]:
        print(f"  positions {start:4d}–{end:4d}  mismatches={miss}")

    print(f"\nPer-versicle bootstrap closure:")
    section_counts: Counter = Counter()
    for v_name, v_data in sorted(result['versicles'].items(),
        ...
        for instr in v_data['instructions']:
            ...
            if m:
                code = MNEMONIC_TO_CODE.get(m.group(1))
                if code:
                    v_stream.append(code)
        n_hits = len(find_bootstrap_windows(v_stream, args.max_mismatches))
        sec = section_of_versicle(v_name)
        section_counts[sec] += n_hits
        if n_hits:
            print(f"  {v_name:6s}  [{sec:12s}]  {n_hits} closure(s)")

    print(f"\nSection closure density:")
    section_vs: Counter = Counter()
    for v_name in result['versicles']:
        section_vs[section_of_versicle(v_name)] += 1
    for _range, sec, _color in SECTIONS:
        total = section_vs.get(sec, 0)
        hits = section_counts.get(sec, 0)
        density = hits / max(total, 1)
        bar = '█' * int(density * 20)
        print(f"  {sec:12s}  {hits:3d}/{total:3d}  {bar} {density:.3f}")
